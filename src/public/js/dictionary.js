...
(() => {
  ...
  function attachCardHandlers(container = document) {
    ...
    container.querySelectorAll(".js-save-term").forEach(btn => {
      ...
      btn._bound = true;
      btn.addEventListener("click", async (e) => {
        const term = btn.dataset.term || (btn.closest(".ld-card")?.querySelector(".card__term-title")?.innerText);
    ...
    container.querySelectorAll('.js-share-term').forEach(btn => {
      ...
      btn._bound = true;
      btn.addEventListener('click', async (e) => {
        const term = btn.getAttribute('data-term') || (btn.closest('.ld-card')?.querySelector('.card__term-title')?.innerText || '');
        ...
        if (navigator.share) {
          try {
            await navigator.share({ title: term, text: 'Definition from LegalHuB: ' + term, url });
            return;
          } catch (err) {
        ...
        if (dialog && typeof dialog.showModal === 'function') {
          ...
          // focus & select link
          if (inputEl) {
            setTimeout(() => { try { inputEl.focus(); inputEl.select(); } catch(e){} }, 40);
          }
          ...
          if (!dialog._shareHandlersBound) {
            ...
            if (closeInline) closeInline.addEventListener('click', () => dialog.close());
            ...
            dialog.addEventListener('cancel', function() { /* allow close */ });
